  ...
  async _init() {
    try {
      ...
      const socketConfig = {
        ...
        browser: this.baileysOptions.browser || ['Ubuntu', 'Firefox', '20.0.00'],
        ...(this.baileysOptions || {}),
      }
      ...
      try {
        ...
        if (Array.isArray(updates)) {
          for (const update of updates) {
            if (update.type === 'protocol' || update.key?.fromMe === false) {
              ...
              if (msg?.protocolMessage?.type === 0) {
                this.emit('message.delete', {
                  message: {
                    ...
                    isGroup: update.key?.remoteJid?.includes('@g.us') || false,
    ...
    sock.ev.on('group-participants.update', async (update) => {
      try {
        ...
        for (const participant of participants) {
          const ctx = {
            jid: id,
            member: { phoneNumber: participant },
            subject,
            groupMetadata,
          }
          if (action === 'add') {
            this.emit('group.add', ctx)
          } else if (action === 'remove') {
            this.emit('group.remove', ctx)
          } else if (action === 'promote') {
            this.emit('group.promote', { ...ctx, participants, action })
          } else if (action === 'demote') {
            this.emit('group.demote', { ...ctx, participants, action })
          }
        }
...
Client.prototype.register = Client.prototype.on
