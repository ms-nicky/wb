...
function applyMessageMethods(client) {
  ...
  client.getRealJid = async (sender) => {
    if (!sender) return sender
    if (sender.endsWith('lid')) {
      try {
        const groups = sock.groupMetadata ? await sock.groupMetadata(sender) : null
        if (groups) return groups.id || sender
        const [result] = await sock.onWhatsApp(sender)
        return result?.jid || sender
      } catch (e) {
        return sender
      }
    }
  ...
  client.getUserId = async (jid) => {
    try {
      const [result] = await sock.onWhatsApp(jid)
      if (result) {
        return { lid: result.jid, jid: result.jid }
      }
      return { lid: jid, jid }
    } catch (e) {
      return { lid: jid, jid }
  ...
  const methodNames = [
    'reply', 'sendReact', 'sendFile', 'sendSticker', 'sendContact', 'sendPoll',
    'sendMessageModify', 'sendMessageModifyV2', 'sendMessageVerify', 'copyNForward',
    'sendProgress', 'sendPtv', 'sendFromAI', 'sendIAMessage', 'sendCarousel',
    'replyButton', 'sendAlbumMessage', 'groupStatus', 'sendMetaMsg', 'pollResult',
    'decodeJid', 'getAdmin', 'lidParser', 'resolveGroupMetadata', 'fetchBlocklist',
    'sendPresenceUpdate', 'readMessages', 'updateBlockStatus', 'getRealJid', 'getUserId',
  ]

  for (const name of methodNames) {
    if (typeof client[name] === 'function') {
      sock[name] = client[name]
    }
  }
}
